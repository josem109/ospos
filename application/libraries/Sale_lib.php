...
class Sale_lib
{
	...
	public function get_totals($taxes, $currency_rate = 1.0, $currency_rate_alternative = 1.0)
	{
		...
		$totals['subtotal2'] = $subtotal2;
		$totals['total'] = $total;
		$totals['total2'] = $total2;
		...
		else
		{
			...
			$cash_total2 = $total2;
			$totals['cash_total'] = $cash_total;
			$totals['cash_total2'] = $cash_total2;
		}
		...
		{
			$amount_due_ves = bcsub($total2, $payment_total2);
			if($amount_due_ves == 0)
			{
				$amount_due = 0;
				$totals['payment_total'] = $subtotal;
	...
	public function add_item(&$item_id, $quantity = 1, $item_location, &$discount = 0.0, $discount_type = 0, $price_mode = PRICE_MODE_STANDARD, $kit_price_option = NULL, $kit_print_option = NULL, $price_override = NULL, $description = NULL, $serialnumber = NULL, $sale_id = NULL, $include_deleted = FALSE, $print_option = NULL, $line = NULL)
	{
		...
		$price_ves = $item_info->unit_price;
